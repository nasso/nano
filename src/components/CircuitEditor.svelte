  ...
  <div class="viewport" use:grow>
    ...
    >
      ...
      {#each circuit.chipsets as chip, i}
        ...
        >
          <ChipNode
            bind:this={chipNodes[chip.name]}
            bind:chip
            {grid}
            state={state?.get(chip.name)}
            on:setpin={({ detail: { pin, value } }) => {
              dispatch("setpin", { pin, value, chip: chip.name });
            }}
          />
        </g>
      ...
      {#each circuit.links as link, i}
        {#if link.to.chip && link.from.chip}
          <polyline
            on:mouseenter={(e) => {
              if (e.buttons === 2) {
            ...
            class="link"
            class:high={state?.get(link.to.chip)?.get(link.to.pin) === true}
            class:low={state?.get(link.to.chip)?.get(link.to.pin) === false}
...
<style lang="scss">
  ...
  .link {
    stroke-width: 2;
    stroke: var(--pin-value-undefined);
